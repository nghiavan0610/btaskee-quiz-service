...
var TimeZoneHoChiMinh *time.Location

func init() {
	TimeZoneHoChiMinh, _ = time.LoadLocation("Asia/Ho_Chi_Minh")
}
...
func StringToTime(dateStr, frm string) (*time.Time, *exception.AppError) {
	parsedTime, err := time.Parse(string(frm), dateStr)
	if err != nil {
		return nil, exception.BadRequest(errors.CodeBadRequest, errors.ErrInvalidTimeFormat)
	}

	return &parsedTime, nil
}

func StringToTimeZone(dateStr, frm string, tz *time.Location) (*time.Time, *exception.AppError) {
	if tz == nil {
		tz = TimeZoneHoChiMinh
	}
...
func StringToUnix(dateStr, frm string) (int64, *exception.AppError) {
...
func FormatToEndOfDay(t string) (*string, *exception.AppError) {
...
func FirstDayOfMonth(str string, frm string) (*time.Time, *exception.AppError) {
...
func LastDayOfMonth(str string, frm string) (*time.Time, *exception.AppError) {
	d, err := StringToTime(str, frm)
	if err != nil {
		return nil, err
	}
...
func RangeDate(dateFrom string, dateTo string, fmt string) (int, *exception.AppError) {
